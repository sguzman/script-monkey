...
ChatGPT normally places the block's native **Copy** control in its header. On a long writing/code block, that control scrolls away even while most of the block is still on screen. This userscript adds a small floating proxy on the block's right edge whenever the native Copy control is offscreen.
...
- When the block header scrolls away, shows a `Copy` button along the visible right edge of the same block.
...
4. A compact `Copy` button should remain available at the block's visible right edge.
